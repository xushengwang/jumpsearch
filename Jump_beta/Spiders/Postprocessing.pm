...
########################################because multiple same #########################################
					...
					my $Jscore = $peptide_E_value + $TagEvalue - $weight * ($mis_cleavage_num + $mod_site_num) * ($peptide_E_value + $TagEvalue);
...
#				print $spout,"\t",$peptide,"\t",$peptide_Jscore->{$spout}->{$peptide},"\t",$all_peptide_info->{$spout}->{$peptide}->{'highest_Jscore'},"\n";
				...
				my $part2_line1 = sprintf("%-2d  %-0.3f    %-0.4f    %-0.4f    %-0.2f     %-10s   %-2d   %-3s    %-0.4f  %-20s    %-20s",$order,$MH,$lsideMass,$rsideMass,$peptide_E_value,$Tag,$TagRank,$TagNum,$Jscore,$first_ref,$peptide);
...
sub get_miscleavage_mod_spout
{
	...
	foreach my $spout (keys %$mainhash)
	{
		...
		my $mods = $self->get_mod_site_number();
		my $miscleavage = $self->get_mis_cleavage_number();
...
{
	my ($self,$$panelty_mainhash, $miscleavage_modification_coeff, $modification_coeff, $miscleavage_coeff) = @_;
	my ($weight, $best_weight, $best_target_decoy_ratio) = (0,0,0);
	my $best_newline;
	
	my $newline = $self->rerank_Jscore($panelty_mainhash,$weight);
	while($weight<0.9)
	{
		($mainhash,$newline) = $self->parse_spOut_data($newline);
		$target_decoy_ratio = $self->calculate_FDR($mainhash);
		if($target_decoy_ratio > $best_target_decoy_ratio)
		{
			$best_weight = $weight;
	...
	return $best_weight,$newline;
}

sub rerank_Jscore{
    my ($self,$panelty_mainhash,$weight)= @_;
...
sub get_db_mis_mod
{
	...
	my $Totoal_peptides = $phash->{'Totoal peptides'};
	my $miscleavage_modification_coeff =  $phash->{'peptide_miscleavage_modification'} / $Totoal_peptides / ($number_miscleavage * $number_mod);
	my $modification_coeff = $phash->{'peptide_modification '} / $Totoal_peptides / $number_mod;
	my $miscleavage_coeff = $phash->{'peptide_miscleavage'} / $Totoal_peptides / $number_miscleavage;

	return ($miscleavage_modification_coeff,$modification_coeff,$miscleavage_coeff);	
}
