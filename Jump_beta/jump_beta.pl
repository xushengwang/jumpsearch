...
########## make multiple dta pathes for a large data set #################	
	my $run_times = int($#$file_array/20000)+1;
...
sub database_creation
{
	...
	#	my $num_mass_region = 20 * $num_dynamic_mod;
		...
		$job->make_createdb_script($database_path,$abs_parameter,$num_mass_region,$num_protein_per_job);

	######### submit job file ##############	
		my $job_num=int($protein_num/$num_protein_per_job)+1;	
		for($i=1;$i<=$job_num;$i++)
		{
			open(JOB,">$database_path/job_db_$i.sh") || die "can not open the job db files\n";
			if($params->{'Job_Management_System'} eq 'LSF')
			{
				print JOB "#BSUB -P prot\n";
				print JOB "#BSUB -q normal\n";
				print JOB "#BSUB -M 20000\n";
				print JOB "#BSUB -R \"rusage[mem=20000]\"\n";			
				print JOB "#BSUB -eo $database_path/$i.e\n";
				print JOB "#BSUB -oo $database_path/$i.o\n";
				print JOB "perl $database_path/create_db.pl $database_path/temp_${i}_${database_basename}\n";
			...
			if($params->{'Job_Management_System'} eq 'SGE')
			{
				...
				print JOB "#\$ \-e $database_path/$i.e\n";
				print JOB "#\$ \-o $database_path/$i.o\n";
				print JOB "perl $database_path/create_db.pl $database_path/temp_${i}_${database_basename}\n";
				print JOB "rm -rf $database_path/temp_${i}_${database_basename}\n";
				close(JOB);
				system(qq(qsub -cwd -pe mpi 4 -l mem_free=4G,h_vmem=2G "$database_path/job_db_$i.sh" >/dev/null 2>&1));
			}
			close(JOB);
		}
	...
	###### Merge all temp_ files into big data file #######################
		...
		$job->make_partialidx_script($database_path);
		...
		{ 		
			...
			Create_Sort_BashFile($parameters,$database_path)							
		}
		...
		my $FileName = "$database_path/merge_mass_index.sh";
		LuchParallelJob($FileName,$cmd,$params->{'Job_Management_System'},"merge_mass_index",$database_path);		
		$cmd = "for i in {0..$merge_job_num} \n do\n cat $peptide_index.".'$i'." >> $peptide_index\n done\n";
		$FileName = "$database_path/merge_peptide_index.sh";
		LuchParallelJob($FileName,$cmd,$params->{'Job_Management_System'},"merge_peptide_index",$database_path);
	...
	####################################################	
		...
		system(qq(rm -rf $database_path/$mass_index.*));
		system(qq(rm -rf $database_path/$peptide_index.*));
		system(qq(rm -rf $database_path/*.o));
		system(qq(rm -rf $database_path/*.e));
		system(qq(rm -rf $database_path/*.sh));
		system(qq(rm -rf $database_path/create_db.pl));
		system(qq(rm -rf $database_path/Create_Partial_Idx.pl));		
