...
############################################################################
				...
				#	print $charge,"\t",$pepseq_core,"\n";
					next if($charge==1 and length($tags->{'tagSeq'})<=2 and length($pepseq_core)<=6);
######################################################################
				#	print "aaaaaaaaa\n";
...
#				print $frag_tolerance,"\t",$mod,"\t",$pepseq,"\t",$tagSeqMatchResult,"\t",$sideMassMatchResult->{'lSide_match'},"\t",$sideMassMatchResult->{'rSide_match'},"\t",$tags->{'tagSeq'},"\t",$tags->{'sideMass'},"\t",$tags->{'hyperP'},"\n";	
				...
				foreach my $aa (keys %$combination_mass)
				{
					if($tags->{'tagSeq'} =~ /$aa/)
					{
						...
						foreach $replace_aa (@$replace_aa)
						{
							...
							my ($tagSeqMatchResult,$sideMassMatchResult) = $self->matchTagSeq($pepseq,$pepseq_core,$mod,\%tags_temp,$frag_tolerance);
							if ($sideMassMatchResult->{'lSide_match'} || $sideMassMatchResult->{'rSide_match'})
							{			
...
=head	
################ Export the tag matching power ##############
		for my $pep (sort {$a<=>$b} keys %$matchResults)
		{
			foreach my $mod (sort {$a<=>$b} keys %{$matchResults->{$pep}})
			{
...
sub matchTagSeq{
	...
	my ($self,  $pepseq, $pepseq_mod, $mod, $tags, $error) = @_;
...
#	$tseq =~ s/L/J/g;
	...
	#	$whole = index($pseq,$tseq,$whole+1);
		...
		$tagSeqMatch = 1;
#		if($tseq eq 'SSEDAESJA')
#		{
#			print $pseq,"\t",$tseq,"\n";
#		}
		$sideMassMatch = $self->checkSideMass($whole,$tags,$pepseq_mod,$mod,$error,$removed_seq);
...
=cut
	...
	if($sideMassMatch->{'lSide_match'}==0 and $sideMassMatch->{'rSide_match'}==0)
	{
		...
		while (1) {	
			...
			$sideMassMatch = $self->checkSideMass_rev($whole,$tags,$pepseq_mod,$mod,$error,$removed_seq);
			last if($sideMassMatch->{'lSide_match'} != 0 || $sideMassMatch->{'rSide_match'} != 0);	
		}
	}
=head	###### ranking tag by p_value rather than using this removing left and right aa method
...
=head	
	...
	my @mod_array = split(//, $mod);
	for(my $k=0;$k<=$#mod_array;$k++)
	{
		print $k,"\n";
		last if($k>$ind);
		if($mod_array[$k]=~/[A-Za-z]/)
		{
			$ind++;
		}
...
####### because the tag was reverse, see tag.pm module, so the N and C terminus was reversed
	...
	#	print $pseq,"\t",$tag->{"tagSeq"},"\t",$mod,"\t",$lSide,"\t",$rSide,"\t",$lSide_mod,"\t",$rSide_mod,"\t",$tag->{'sideMass'},"\t",$result->{'lthreot_Mass'},"\n"; 
...
#			Discard those peptides with charge = 1, No tag, and peptide length < 8
			my $length_peptide = length($pepseq) - 4;
			next if($charge==1 and $length_peptide <= 8);
# 			Discard those peptides with length <= 5
			next if($length_peptide<=5);
...
###### Version 1.12 fix a bug: using frag tolerance instead of mass tolerance	
	...
	my $exp_mz = $self->get_exp_mz();
######### Version 2.00 fix a bug: for low resolution, using exp mass range rather than theoretical mass range	
...
#	}
	...
	my $peptide_pvalue=$hyper->Hypergeometric($total_number,$total,$exp_mass_num,$matched);	
#	my $peptide_pvalue=$hyper->Hypergeometric($total_number,$exp_mass_num,$total,$matched);	

	my $log_peptide_pvalue = sprintf("%.6f",-log($peptide_pvalue)/log(10));
################### end ##############################	
...
###### output modification information ###############################
	...
	foreach (keys %$parameter)
	{
		...
		if($_ =~/dynamic_/ and $parameter->{$_} != 0)
		{
			...
			if($_=~/S|T|Y/)
			{
				$mod_symbol_used = join("|",$mod_symbol_used,$mod_symbol[$i]);
			}
			$i++;
		}
	}
	...
	{	
		...
		{	
			...
			{	
				...
				my $pep_pvalue = 0;
				print $pep,"\t",$total,"\t",$matched,"\t",$self->{'Matched_ion_num'},"\t", $self->{'Theor_ion_num'},"\t",$pep_pvalue,"aaaaaaaa\n";				
				if(!defined($PeptideEvaluehash{$total}{$matched}))
				{	
					$pep_pvalue = sprintf("%.2f",$self->get_peptide_pvalue($self->{'Matched_ion_num'} , $self->{'Theor_ion_num'}));
					print $total,"\t",$matched,"\t",$self->{'Matched_ion_num'},"\t", $self->{'Theor_ion_num'},"\t",$pep_pvalue,"bbbbbbb\n";
...
#		{
			foreach my $pep (sort {$a<=>$b} keys %{$sort_results{$pvalue}})
			{
				...
				foreach my $protein (keys %{$sort_results{$pvalue}{$pep}})
				{
					if($j==1)
					{
						my @result_array = split(/\s+/,$sort_results{$pvalue}{$pep}{$protein});
...
#		{
			foreach my $pep (sort {$a<=>$b} keys %{$sort_results{$pvalue}})
			{
				...
				foreach my $protein (keys %{$sort_results{$pvalue}{$pep}})
				{
					if($j==1)
					{
						printf OUTPUT ("%2d    %10s\n",$i,$sort_results{$pvalue}{$pep}{$protein});
						$j++;
					}
					else
					{
...
####################################################################### if the new tag is longer than the tag with highest score, then calculate a coefficient for the tag score 
				...
				print "Before:",$highest_Tag_seq,"\t",$TagSeq,"\n";		
				next if (length($highest_Tag_seq)>length($TagSeq) and  $highest_Tag_seq=~/$TagSeq/);
############# If the highest_Tag_seq is derived from y ion, there are some tags derived from b ions
				...
				print "After:",,$highest_Tag_seq,"\t",$TagSeq,"\n";	
				my @substr = $self->lc_substr($highest_Tag_seq,$TagSeq);
				if(scalar(@substr)==0)
				{
...
################ add penalty for mis-cleavage #####################
=head
		if(length($longest_Tag_seq)<4 and $charge==2)
		{
			my $mis = ($Peptide =~ s/([KR][A-OQ-Z])/$1/g) || 0;
			
			for(my $k=1;$k<$mis;$k++)
			{
				$WE->{$Peptide}->{'Weighted_Evalue'} = $WE->{$Peptide}->{'Weighted_Evalue'} - $WE->{$Peptide}->{'Weighted_Evalue'} * 0.10;
			}
		}
=cut		
##################################################################### 		
		
#		print $WE->{$Peptide}->{'Weighted_Evalue'} ,"\n";
####### 6/11/2013   Add weighted score of H3PO4-neutral loss 
		if($parameter->{'pho_neutral_loss'})
		{
			if($self->get_pho_neutral_loss())
			{
				...
				if($peptide=~/$mod_symbol_used/)
				{
					$WE->{$Peptide}->{'Weighted_Evalue'} = $WE->{$Peptide}->{'Weighted_Evalue'} + $parameter->{'pho_neutral_loss'} * $WE->{$Peptide}->{'Weighted_Evalue'}; 
				}
			}
		}			
