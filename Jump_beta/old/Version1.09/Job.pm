...
#	\$deisotope->print_mass_error("\$dtafile.mserr");
	...
	if(scalar(\%\$updatedresults)<1)
	{
		\$updatedresults = \$search->SearchWithoutTag(\$searchmassresults);
		\$search->WriteResults4NoTags(\$searchfile,\$updatedresults,\$tag_rank_num);
	}
	else
	{
...
GetOptions('-m=i'=>\\\$m,
	...
	#{ 				
		...
		for(\$i=1;\$i<=\$job_num;\$i++)
		{
			...
			my \$filename = "\$curr_dir/\$dta_path/.database/temp_\${i}_\${databasename}_*_*_mass_\$m";
			my \$cmd = qx(ls -1 \$filename);
			next if(\$cmd eq \"\");
			my \@cmd_array = split("\\n",\$cmd);
			#my \$mass_out = "\$curr_dir/\$dta_path/.database/temp_\${i}_\${databasename}_mass_\$m";
			my \$mass_out = \$cmd_array[0];
			my \$mass_retrieve = retrieve(\$mass_out);
			my \$pep_out = "\$curr_dir/\$dta_path/.database/temp_\${i}_\${databasename}_pep_\$m";
			my \$pep_retrieve = retrieve(\$pep_out);
					
			print "\$dta_path/.database/temp_\${i}_\${databasename}_mass\\n";
...
########### *2 is because ID comprise of decoy (generated by makeDB module) and target 					
			...
			if(\$m==1)
			{
				my \$pro_out = "\$curr_dir/\$dta_path/.database/temp_\${i}_\${databasename}_pro";
...
sub GetPreviousFilesMass{
	...
	for(my \$j=0;\$j<\$m;\$j++){
		for(my \$i=1;\$i<\$job_num;\$i++){
			my \$filename = "\$curr_dir/\$dta_path/.database/temp_\${i}_\${databasename}_\*_\*_mass_\$j";
			my \$cmd = qx(ls -1 \$filename);
			next if (\$cmd eq \"\");
			my \@cmd_array = split("\\n",\$cmd);
			\$cmd = \$cmd_array[0];
