...
#			my ($tagSeqMatchResult,$sideMassMatchResult) = $self->matchTagSeq($pepseq,$mod,$tags,$error);
			...
			{	
				...
				$matchResults->{$pep}->{$mod}->{'rankP'} = $tags->{'rankP'};
				$matchResults->{$pep}->{$mod}->{'hyperP'} = $tags->{'hyperP'};
			}
...
#	print OUTPUT "Tag E_value = ", $tag_e_value,"\n","Tag Rank Number = ",$tag_rank_num,"\n","Tag Side Mass = ",$self->get_sideMass(),"\n"; 
	...
	#			{
					my $pep_pvalue = $self->get_peptide_pvalue($matches->{$tag}->{$pep}->{$mod}->{'pepseq'},$mod);
					...
					$pep_pvalue,$matches->{$tag}->{$pep}->{$mod}->{'tagSeq'},$matches->{$tag}->{$pep}->{$mod}->{'rankP'},$matches->{$tag}->{$pep}->{$mod}->{'sideMass'},$matches->{$tag}->{$pep}->{$mod}->{'proteinid'},
