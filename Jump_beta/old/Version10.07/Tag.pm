...
#		print $mz_pos1,"bb\n"; 
		...
		my %mass_position;
		$self->iterate($mz_pos1,\%tag_AA_hash,\$tag,$position,\%mass_position,\@tag_array,\@mz,$tag_peaks,\%tag_hyper_pvalue_hash,\$hyper_p);		
...
#	my $rank_hyper_pvalue = $tag->{'hyper_p'}>0 ? -10*log($tag->{'hyper_p'}) :  0;
	my $rank_pvalue = -log($tag->{'p_value'});
...
####### it changes back because of low resolution
	...
	{
		my $hyperp = $tag->{'hyper_p'} =sprintf("%.11f",$tag->{'hyper_p'});
		$rank_hyper_pvalue = -log($hyperp);
	}
######################################	
...
sub select_top_rank_pvalue
{
	...
	foreach my $tag (@$tag_array)
	{
		if($tag->{'p_value'}<$min_rank_pvalue)
		{
			...
			$select_rank_pvalue = $tag->{'p_value'}>0 ? -10*log($tag->{'p_value'}) :  0;
			$select_hyper_pvalue = $tag->{'hyper_p'}>0 ? -10*log($tag->{'hyper_p'}) :  0;
...
sub iterate
{
	my ($self,$pos1,$tag_AA_hash,$tag,$position,$mass_position,$tag_array,$mz,$tag_peaks,$tag_hyper_pvalue_hash,$hyper_p)=@_;
	my $flag = 0;
...
### The first if to difine the first amino acid .... i.e. N has to control ...		
...
#			print $pos,"\t",$pos1,"\t",$aa,"\t",$hyper_pvalue,"\t",$$tag,"\t",$p_value,"\n";
						
			push (@$tag_array,\%tag_temp);

			$flag = 1;
			$mass_position->{$pos1}=$position;	
			$self->iterate($pos,$tag_AA_hash,$tag,$position,$mass_position,$tag_array,$mz,$tag_peaks,$tag_hyper_pvalue_hash,$hyper_p);
		}
		else
		{
			if(defined($mass_position->{$pos1}))
