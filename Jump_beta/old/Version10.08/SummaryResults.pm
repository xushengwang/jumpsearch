...
sub score_distribution
{
	...
	my $score;
	foreach my $file(keys %{$mainhash}){
		my $pep_score = sprintf("%d",($mainhash->{$file}->{"outhash"}->{"Order"}->{1}->{"peptide_E_value"}));
...
sub FDR_calculation{
	...
	foreach my $file(keys %{$mainhash}){	
		...
		push @{$PeptideEvals->{int($mainhash->{$file}->{"outhash"}->{"Order"}->{1}->{"peptide_E_value"}+0.5)}},$file;	
	}
...
#			$DownNonDecoy= $NonDecoryRefs->{$thresholds[$i]};	
		...
		if($DownkeysDecoy+$UpkeysDecoy!=0){
			$scores_FDR_hash->{"Tag_E_value"}->{"$thresholds[$key]"}->{"spesificity"} = $DownkeysDecoy / ($DownkeysDecoy+$UpkeysDecoy);
		}
		else{
			$scores_FDR_hash->{"Tag_E_value"}->{"$thresholds[$key]"}->{"spesificity"} =0;}
...
sub CalcPeptideEvalue{
	...
	my ($self,$mainhash,$scores_FDR_hash,$PeptideEvals,$field)=@_;
	...
	{ #Go through the files associated with each Tag Evalue 
		...
		foreach my $spout(@{$PeptideEvals->{$key}}){													
			...
			if($mainhash->{$spout}->{"outhash"}->{"Order"}->{1}->{"peptide_E_value"}>$key){
				$nbSup++;
			}					 			 			 
		}
		 ...
		 if($nbNonDecoyRef>0){ 
			$scores_FDR_hash->{"peptide_E_value"}->{"$key"}->{"FDR"} = $nbDecoyRef / $nbNonDecoyRef;										
		 }
		 else #in case for one key all the files have only ##Decoy top refs.
		 {
			 $scores_FDR_hash->{"peptide_E_value"}->{"$key"}->{"FDR"} =-$nbDecoyRef;
		 }			 
############## changed #################################
		
		 $scores_FDR_hash->{"peptide_E_value"}->{"$key"}->{"nbDecoy"} = $nbDecoyRef;
		 $scores_FDR_hash->{"peptide_E_value"}->{"$key"}->{"nbTaget"} = $nbNonDecoyRef;		 
		 $scores_FDR_hash->{"peptide_E_value"}->{"$key"}->{"Coverage"}= ($nbDecoyRef+$nbNonDecoyRef)/$test_number;
...
#			$DownNonDecoy= $NonDecoryRefs->{$thresholds[$i]};	
		...
		if($UpKeyNonDecoy+$DownNonDecoy!=0){		
			$scores_FDR_hash->{"peptide_E_value"}->{"$thresholds[$key]"}->{"sensitivity"} = $UpKeyNonDecoy/ $total_target;
		}
		else
		{
			$scores_FDR_hash->{"peptide_E_value"}->{"$thresholds[$key]"}->{"sensitivity"}=0;
		}
		if($DownkeysDecoy+$UpkeysDecoy!=0){
			$scores_FDR_hash->{"peptide_E_value"}->{"$thresholds[$key]"}->{"spesificity"} = $UpkeysDecoy / $total_decoy;
			}
		else{
			$scores_FDR_hash->{"peptide_E_value"}->{"$thresholds[$key]"}->{"spesificity"} =0;}
...
#			$DownNonDecoy= $NonDecoryRefs->{$thresholds[$i]};	
		...
		if($DownkeysDecoy+$UpkeysDecoy!=0){
			$scores_FDR_hash->{"XCorr"}->{"$thresholds[$key]"}->{"spesificity"} = $UpkeysDecoy / $total_decoy;
			}
		else{
			$scores_FDR_hash->{"XCorr"}->{"$thresholds[$key]"}->{"spesificity"} =0;}
...
sub compare_psearch_sequest_scores
{
	...
	my $path = $self->get_path();
	my $runhash = $self->parse_outfile($path);
	my $mainhash = $self->parse_spOut_files($path);
	...
	foreach my $file (keys %merge_files)
	{
		...
		next if (!defined($mainhash->{$file}->{"outhash"}->{"Order"}->{1}->{"peptide_E_value"}) and !defined($runhash->{$file}->{'XCorr'}));
		...
		if(defined($mainhash->{$file}->{"outhash"}->{"Order"}->{1}->{"peptide_E_value"}))
		{

			$comp_score->{$file}->{"Escore"} = $mainhash->{$file}->{"outhash"}->{"Order"}->{1}->{"peptide_E_value"};		
		}
		else
		{
		...
		if($mainhash->{$file}->{"outhash"}->{"Order"}->{1}->{"peptide"} eq $runhash->{$file}->{'intpep'})
		{
