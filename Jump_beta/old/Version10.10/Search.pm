...
## multiple tag #######	
	...
	print OUTPUT "Database = ", $parameter->{'database_name'}, "\n";
	print OUTPUT "Precursor mass = ",$self->get_precMass(),"\n";
	print OUTPUT "Precursor matched peptides = ", $self->{'matched_prec_peptide_num'},"\, ","Peptides mass tolerance = $parameter->{'peptide_tolerance'}\n";
...
#						my $pep_pvalue = $self->get_peptide_pvalue($peptideseq,$mod) + $matches->{$tag}->{$pep}->{$mod}->{$parameter->{'tag_select_method'}};
	...
	# version 1.03 $matches->{$tag}->{$pep}->{$mod}->{$parameter->{'tag_select_method'}}
					...
					$peptide_pvalue{$pep},$matches->{$tag}->{$pep}->{$mod}->{'tagSeq'},$matches->{$tag}->{$pep}->{$mod}->{$parameter->{'tag_select_method'}},$matches->{$tag}->{$pep}->{$mod}->{'sideMass'},$ions{$pep},$matches->{$tag}->{$pep}->{$mod}->{'proteinid'},
					$matches->{$tag}->{$pep}->{$mod}->{'matchPeptide_orig'});
					$sort_results{$peptide_pvalue{$pep}}{$tag}{$matches->{$tag}->{$pep}->{$mod}->{'matchPeptide_orig'}}{$matches->{$tag}->{$pep}->{$mod}->{'proteinid'}}=$results;
		...
		foreach my $tag (keys %{$sort_results{$pvalue}})
		{
			foreach my $pep (sort {$a<=>$b} keys %{$sort_results{$pvalue}{$tag}})
			{
				...
				foreach my $protein (keys %{$sort_results{$pvalue}{$tag}{$pep}})
				{
					if($j==1)
					{
						my ($M_H, $lSideMass, $rSideMass, $PeptideEvalue, $TagSeq, $TagEvalue, $ions ,$Reference, $Peptide) = (split/\s+/,$sort_results{$pvalue}{$tag}{$pep}{$protein})[0, 1, 2, 3, 4, 5, 6, 8, 9,10];
...
sub WriteResults4NoTags
{
	...
	#		next if (!defined($peptideseq));
	#		my $peptideseq = $matches->{$pep}->{$mod}->{'pepseq'};
			print $peptideseq,"\n";
			if(!$peptide_pvalue{$pep})
			{					
				my $pep_pvalue = $self->get_peptide_pvalue($peptideseq,$mod);
			...
			$peptide_pvalue{$pep},$matches->{$pep}->{$mod}->{'tagSeq'},$matches->{$pep}->{$mod}->{$parameter->{'tag_select_method'}},$matches->{$pep}->{$mod}->{'sideMass'},$matches->{$pep}->{$mod}->{'proteinid'},
...
{   
    ...
    {   
        ...
        $WE->{$Peptide}->{'Weighted_Evalue'} = $PeptideEvalue + $longest_Tag_Evalue;
        my @TagSeq_Evalue       = map { [$_->[5], $_->[6]] } @sorted_lines;
        for my $each_pair (@TagSeq_Evalue)
        {   my ($TagSeq, $TagEvalue) = @$each_pair;
            $WE->{$Peptide}->{'Weighted_Evalue'} += ($TagSeq =~ /$longest_Tag_seq/) ? 0.5 * $TagEvalue : $TagEvalue;
        }
    }
    return $WE;
}
