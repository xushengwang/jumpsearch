...
sub MS1_deisotope
{
	my ($self,$select_mz,$charge)=@_;
	my $mz_hash = $self->{'_deisotope_mz_hash'};
	my $C = $self->get_C_value();

	my $parameter = $self->get_parameter();
	my $peptide_tolerance = $parameter->{'peptide_tolerance'};
...
######### deisotoping #################
		my $updated_mz = $self->MS1_deisotope($mz,$realcharge->{$specscan});
#######################################
