...
## multiple tag #######	
	...
	print OUTPUT "Database = ", $parameter->{'database_name'}, "\n";
	print OUTPUT "Precursor mass = ",$self->get_precMass(),"\n";
	print OUTPUT "Precursor matched peptides = ", $self->{'matched_prec_peptide_num'},"\, ","Peptides mass tolerance = $parameter->{'peptide_tolerance'}, Peptides mass tolerance units = $parameter->{'peptide_tolerance_units'}\n";
...
sub WriteResults4NoTags
{
	...
	#		my $peptideseq = $matches->{$pep}->{$mod}->{'pepseq'};
			...
			my $results = sprintf("%-0.3f    %-0.4f    %-0.4f    %-0.2f     %-10s   %-0.2f    %-0.4f    %-10s  %-30s    %-30s", 
