...
sub score_distribution
{
	...
	foreach my $file(keys %{$mainhash}){
		...
		my $pep_score = sprintf("%d",($mainhash->{$file}->{"outhash"}->{"Order"}->{1}->{"Weight_E_value"}+0.5));
...
#			$DownNonDecoy= $NonDecoryRefs->{$thresholds[$i]};	
		...
		if($DownkeysDecoy+$UpkeysDecoy!=0){
			$scores_FDR_hash->{"Weight_E_value"}->{"$thresholds[$key]"}->{"spesificity"} = $UpkeysDecoy / $total_decoy;
			}
		else{
			$scores_FDR_hash->{"Weight_E_value"}->{"$thresholds[$key]"}->{"spesificity"} =0;}
...
sub compare_psearch_sequest_scores
{
	...
	my $path = $self->get_path();
	my $runhash = $self->parse_outfile($path);
	my $mainhash = $self->parse_spOut_files_v4($path);
