...
sub create_pepseq_index_file
{
	...
	open(PEPSEQOUTPUT,">>$seqIndex") or die "Error opening file :$peptideIndex, $!\n";		
	foreach my $peptideid (sort {$a<=>$b} keys %$peptidehash)
	{
		print PEPSEQOUTPUT pack("L A255",$peptideid,$peptidehash->{$peptide}->{'seq'});
	}
	close(PEPSEQOUTPUT);
}

sub create_pep_index_file
{
	my ($self, $masshash, $peptidehash, $peptideIndex) = @_;
...
#		@vals = unpack("L L L A255",$val);
		...
		if($vals[2]==$massId){
############## changed by seq ################
			my ($seqentry,$index)= $self->dicoSearch($seqfilename,$seqnbElements,"L L",$seqentrySize,$vals[0],2);
