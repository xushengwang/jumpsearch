...
sub generate_hash_dta
{
	...
	my $win = $parameter->{'precursor_mass_window'}/2;
	...
	foreach (@$index_array)
	{
		...
		$number++;
		my $mslevel=0;
		print "\r  Getting scan information: $number of $last_scan scans                                                     ";
...
sub strongest_prec{
	...
	my $mz = $$hash{'xml_mz'};
	my ($low, $high) = ($mz-$win-.25, $mz+$win+.25); #DMD Changed Dec 9, 2008 Added additional left and right tolerances
