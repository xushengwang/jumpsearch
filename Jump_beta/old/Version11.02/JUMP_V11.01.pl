...
######### submit job file ##############	
	...
	for($i=1;$i<=$job_num;$i++)
	{
		...
		if($params->{'Job_Management_System'} eq 'SGE')
		{
			...
			print JOB "perl $curr_dir/$dta_path/create_db.pl $database_path/temp_${i}_${database_basename}\n";
			close(JOB);
			system(qq(qsub -cwd "$dta_path/job_db_$i.sh" >/dev/null 2>&1));
		}		
	}
...
###### Merge all temp_ files into big data file #######################
	...
	print "  You submit 20 jobs for sorting index files\n";
	#Waint For the jobs until they get finished
	Check_Job_stat("sort_","20");
...
## end of change
		...
		{
			my $check_command = "ls -l $curr_dir\/$dta_path\/\*.spout \| wc -l";
			

			my $outfile_num=qx[$check_command];
			chomp $outfile_num;
			print "\r  Searching $outfile_num out of $#dta dta files          ";
		}
