...
#		print $dtafile,"\n";
		...
		foreach my $order (keys %{$PIP->{$specscan}})
		{
			...
			if($charge != 0 and ($PIP->{$specscan}->{$order}->{'pip'}*100)>$parameter->{'percentage_ppi'})
			{
	...
	print "  Decharged $dta_number precursor ions: 1=$charge_number{'1'} 2=$charge_number{'2'} 3=$charge_number{'3'} 4=$charge_number{'4'} 5=$charge_number{'5'}\n";
