package Spiders::MSMSTheoSpectrum;
...
sub getTermIons
{
  ...
  foreach my $frag (keys(%{$spectrum{mass}{term}})){
    for (my $i = 0; $i < @{$spectrum{ionType}{$frag}}; $i++){
      my ($series, $charge) = ($this->getFragType($frag))[0,1];
      my $terminus = ($this->getSeries($series))[0];
	...
	if (defined($spectrum{match}{term}{$frag}[$j])){
      ...
      push(@termIons, {ionType=>$ionType, charge=>$charge, series=>$series, terminus=>$terminus, names=>$names, masses=>[@masses], massType=>$this->massType(), intensityIndex=>$spectrum{intensityIndex}, massIndex=>$massIndex, matches=>(defined($spectrum{match}) ? [@matches] : undef)});
    }
  }
#### changed by xusheng;
 # @termIons = sort {$this->cmpFragTypes($a->ionType(), $b->ionType())} @termIons;
  return @termIons;
...
sub getInternIons
{
  ...
  foreach my $frag (keys(%{$spectrum{mass}{intern}})){
    ...
    push(@internIons, {ionType=>$ionType, charge=>1, names=>[@names], masses=>[@masses], massType=>$this->massType(), massIndex=>$massIndex, intensityIndex=>$spectrum{intensityIndex}, matches=>(defined($spectrum{match}) ? [@matches] : undef)});
  }
#### changed by xusheng;
#  @internIons = sort {$this->cmpFragTypes($a->ionType(), $b->ionType())} @internIons;
  return @internIons;
...
sub getMass{
	...
	foreach $aa (keys %$h){
		if(!defined($aaMW->{$aa}))
		{
			print "your sequences contain unkown amino acid: $aa!\n";
			next;
		}
		$mw += $h->{$aa} * $aaMW->{$aa};
	}
