...
#			}
			$peptide_num++;
			if($peptidehash->{$peptide}->{'seq'} =~ /([\@\#\%\^\&\*\?\~\!\(\)\{\}\[\]\:\;\'\<\>]+)/ and $peptidehash->{$peptide}->{'seq'} =~ /([KR][A-OQ-Z])/)
			{
				$mis_cleavage_mod++;
			}
			elsif($peptidehash->{$peptide}->{'seq'} =~ /([\@\#\%\^\&\*\?\~\!\(\)\{\}\[\]\:\;\'\<\>]+)/)
			{
				$mods++;
			}
			elsif($peptidehash->{$peptide}->{'seq'} =~ /([KR][A-OQ-Z])/)
			{
				$mis_cleavage++;
			}
			else
	...
	print MOUTPUTTXT "peptide_number:",$peptide_num,"\n","peptide_normal:",$norm_peptide,"\n","peptide_miscleavage:",$mis_cleavage,"\n","peptide_modification:",$mods,"\n","peptide_miscleavage_modification:",$mis_cleavage_mod,"\n";
	close(MOUTPUTTXT);
	close(MOUTPUT);
	close(PEPOUTPUT);	
}
