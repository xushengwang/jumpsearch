...
### create a script to sum the numbers #############	
	...
	summary_db(\@summaryfiles,"$mass_index.sdx");
...
system(qq(cp sequest.params $dta_path >/dev/null 2>&1));
my $job = new Spiders::Job;
...
my $dta_num_per_file = 120/($num_dynamic_mod*2);
...
$spout->printPepXML(\%seqParaHash, \%outInforHash, "$curr_dir/$dta_path", $outXML, 1);
