...
print RUNSHELL <<EOF;	
#!/usr/bin/perl  -I /home/xwang4/scripts/beta -I /usr/local/lib/perl5
use Getopt::Long;
...
#	my \$dtafile = abs_path(\$dtafile);
		...
		my \$ms2_signal_noise_ratio = \$deisotope->MS2_deisotope();
#		\$deisotope->print_mass_error("\$dtafile.mserr");
	...
	my \$pho_neutral_loss = \$consolidation->Normalize_Consolidation();
...
#	\$sequest->set_sequest_path("/home/xwang4/scripts");
...
#		undef \%matched_tag;
	...
	if(scalar( keys \%\$updatedresults)<1)
	{
		...
		\$search->WriteResults4NoTags(\$searchfile,\$updatedresults,\$tag_rank_num,\$prec_peak_int,\$ms2_signal_noise_ratio);
	}
	else
	{
...
sub make_createdb_script
{
	...
	print DB "#!/usr/bin/perl -I /home/xwang4/scripts/beta\n";
...
print PIDX <<EOF;
#!/usr/bin/perl -I /home/xwang4/scripts/beta
...
my (\$m,\$job_num,\$dta_path,\$mass_index,\$peptide_index,\$protein_index,\$databasename,\$prot_job_num);
GetOptions('-m=i'=>\\\$m,
		'-job_num=i'=>\\\$job_num,
		'-dta_path=s'=>\\\$dta_path,
...
####### *2 is because ID comprise of decoy (generated by makeDB module) and target 
				...
				{
					\$peptide_hash->{\$j}->{'proteinID'} = \$pep_retrieve->{\$k}->{'proteinID'}+(\$i-1)*\$num_pro_per_job;				
				}
			}
		}
