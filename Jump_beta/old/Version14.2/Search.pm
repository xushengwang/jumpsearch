...
#	print $peptide,"xxxxx\n",$_,"\n";
#	foreach (sort {$a<=>$b} @{$spectrum{'mass'}{'term'}{$_}})
#	{
#		print $_,"fff\n";
#	}
...
#	my $total_number = $total_location > $total_location_alt ? $total_location : $total_location_alt; 
	...
	my $log_peptide_pvalue = 0;
	
	if($parameter->{'matching_method'} == "hyper_p")
	{
...
#	my $peptide_pvalue=$hyper->Hypergeometric($total_number,$exp_mass_num,$total,$matched);	
	...
	elsif($parameter->{'matching_method'} == "rank_p")
	{
		my @sel_array;
		my @unsel_array;
	...
	elsif($parameter->{'matching_method'} == "comb_p")
	{
		my @sel_array;
		my @unsel_array;
...
#	my $peptide_pvalue=$hyper->Hypergeometric($total_number,$exp_mass_num,$total,$matched);	
		...
		$log_peptide_pvalue = ($log_peptide_rankp + $log_peptide_hyperp) / 2;
	}
	else
	{
...
######################### Output the selected candidate peptide ####################
		...
		} keys %$WeightedEvalue) #TODO: any better sort sub?
    {
        ...
        my @References  = reverse sort keys %$refs;
...
#				print $peptideseq,"\t",$mod,"\t",$matched,"\t",$total,"\t",$self->{'Matched_ion_num'},"\t",$self->{'Theor_ion_num'},"\t",$pep_pvalue,"\t",$peptide_pvalue{$pep},"\n";
			...
			my $results = sprintf("%-0.3f    %-0.4f    %-0.4f       %-0.2f         %-6s     %-0.2f      %-0.4f       %-6s     %-20s    %-20s", 
