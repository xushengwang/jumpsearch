        ...
        while (<IN>)
        {
            ...
            $peptide =~ s/([A-Z\-])\.([A-Z\@\#\*\^\~\$]+)\.([A-Z\-])/$2/;
                        ...
                        if ( defined($$outInfor{$outfile}{'fullPeptide'}[$i-1]) and $peptide eq $$outInfor{$outfile}{'fullPeptide'}[$i-1]) #same peptide
                        {
                                ...
                                #alternativ proteins
                                $count++;
                                $$outInfor{$outfile}{'altPro'}[$i][$count]=$vals[8];
                        ...
                        $$outInfor{$outfile}{'protein'}[$i]=$vals[8];
                        ...
                        $$outInfor{$outfile}{'WeightedEvalue'}[$i]=$vals[6];
...
sub printPepXML
{
	...
	my ($now_date, $now_time) = ($self->{'now_date'},$self->{'now_time'});
        ...
        print XML '<msms_pipeline_analysis date="',"$now_date $now_time",'" ','summary_xml="',$run,$outXML,'.pepXML">',"\n";
