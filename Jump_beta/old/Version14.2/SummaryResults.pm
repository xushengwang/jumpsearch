...
# Version 1.05 change the format of spout
...
sub parse_outfile_multiple
{
	...
	foreach my $outfile (@files)
	{
       ...
       open (IN, $fullpath);
        #test03.10240.10240.2.out
		$outfile =basename($outfile);
        $outfile =~ s/(\.)(\d+)(\.)(\d)(\.out)/$1$2$3$4$5/;
        my ($scan,$charge) = ($2,$4);

        seek(IN,0,0);
        # (M+H)+ mass = 1072.65321 ~ 0.0075 (+2), fragment tol = 0.0200, MONO/MONO
        my @MH=grep(/^\s*\(M\+H\)\+\s*mass/,<IN>);
        if (defined($MH[0]))
        {
                $MH[0] =~ /^\s*\(M\+H\)\+\s*mass\s*\=\s*([0-9\.]+)\s*\~\s*([0-9\.]+)\s*\(\+(\d)\)/;
                $outInfor{$outfile}{'MHmass'}=$1;
        }
        else
        {
                print 'warning: no line matched "(M+H)+ mass)"',"\n";
        }
    ...
    #    $outInfor{$outfile}{'hitShown'}=0;
        for (my $i=0; $i<scalar(@hits) and $i<$maxHitCondiered; $i++ )
        {
    #            $outInfor{$outfile}{'hitShown'}++;
                #  #   Rank/Sp      Id#     (M+H)+    deltCn   XCorr    Sp    Ions  Reference                            Peptide
                #  1.   1 /  8      75601 1148.70398  0.0000  1.4579   137.9   7/18  ##Decoy__sp|Q6P387|CP046_HUMAN   +1  L.ANSLSAK.L
    ...
    #            my $tryptic = istryptic($peptide);
    #            $tryptic -= 1 if ($tryptic>1);
				$outfile =~ s/\.out//;  
				$outfile =~ s/\.0/\./g;
                $outInfor{$outfile}{$i+1}{'origpeptide'}=$peptide;					
                $peptide =~ s/([A-Z\-])\.([A-Z\@\#\*\^\~\$]+)\.([A-Z\-])/$2/;
			
                $outInfor{$outfile}{$i+1}{'prAA'}=$1;
                $outInfor{$outfile}{$i+1}{'nxAA'}=$3;

                $outInfor{$outfile}{$i+1}{'mis'}= ( $outInfor{$outfile}{$i+1}{'origpeptide'}=~ s/([KR][A-OQ-Z])/$1/g) || 0;
                #$$outInfor{$outfile}{'mod'}[$i+1]= ($$outInfor{$outfile}{'origpeptide'}[$i+1] =~ s/([\@\#\*\^\~\$]+)/$1/g) || 0;

                #$protein =~ s/\#\#([a-zA-Z0-9\.\_\-\|\:]+)/$1/;
                $outInfor{$outfile}{$i+1}{'rank'}=$rank;
#                $outInfor{$outfile}{'sprank'}[$i+1]=$spRank;
                $outInfor{$outfile}{$i+1}{'id'}=$id;
                $outInfor{$outfile}{$i+1}{'expmass'}=$expmass;
                $outInfor{$outfile}{$i+1}{'xcorr'}=$XCorr;
                $outInfor{$outfile}{$i+1}{'spscore'}=$Sp;
                $outInfor{$outfile}{$i+1}{'red'}=$red;
        #        $outInfor{$outfile}{'tryptic'}[$i+1]=$tryptic;

                $outInfor{$outfile}{$i+1}{'deltacn'}=$c;
                $outInfor{$outfile}{$i+1}{'deltacn'}=0;
                $outInfor{$outfile}{$i+1}{'deltacnstar'}=0;            #other options? or leave blank?
				$outInfor{$outfile}{$i+1}{'protein'}=$protein;
                $e =~ /(\d+)\/(\d+)/;
                $outInfor{$outfile}{$i+1}{'matchIons'}=$1;
                $outInfor{$outfile}{$i+1}{'totalIons'}=$2;
		}
	}
	return \%outInfor;
}
