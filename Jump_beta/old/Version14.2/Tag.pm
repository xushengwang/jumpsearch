...
#################################################################################################################	
	...
	#sort by hypergeometric p value	
			...
			for(my $i=0;$i<$#tag_array; $i++)
			{
				...
				{
					$tag_array[$i]->{'comb_p'} = sprintf("%.10f",sqrt($tag_array[$i]->{'hyper_p'} * $tag_array[$i]->{'p_value'}));
				}
			}
			@sorted_tag = sort {$a->{'comb_p'}<=>$b->{'comb_p'}} @tag_array;
#			shift @sorted_tag;
			foreach (@sorted_tag)
			{
				
				print $_->{'tag'},"\t",$_->{'hyper_p'},"\t",$_->{'p_value'},"\t",$_->{'comb_p'},"\n";
			}
...
#	{
		...
		my $comb_p = ($rank_pvalue+$rank_hyper_pvalue)/2;
...
#   $number_matched_peaks:  
	...
	my $hyper_pvalue=$hyper->Hypergeometric($mass_poss_bin,$amino_acid_num,$number_peak_window,$number_matched_peaks);
