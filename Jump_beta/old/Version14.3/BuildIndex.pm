...
#			}
			...
			$peptide_type->{$mis_cleavage_num}->{$mod_site_num}++;
...
#	print MOUTPUTTXT "miscleavage_num\tmodification_num\tpeptide_number\n";
	foreach my $mis_num (sort {$a<=>$b} keys %$peptide_type)
	{
		foreach my $mod_num (sort {$a<=>$b} keys %{$peptide_type->{$mis_num}})
		{
			if($digestion eq "partial")
			{
				print MOUTPUTTXT "1\t",$mis_num,"\t",$mod_num,"\t",$peptide_type->{$mis_num}->{$mod_num},"\n";
			}
			elsif($digestion eq "full")
			{
				print MOUTPUTTXT "0\t",$mis_num,"\t",$mod_num,"\t",$peptide_type->{$mis_num}->{$mod_num},"\n";			
			}
		}
	}
