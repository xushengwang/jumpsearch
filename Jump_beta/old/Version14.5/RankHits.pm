...
#	my $mainhash_all = $self->parse_spOut_files_v5($folder);
	my ($sum_target,$cutoff_score) = $self->calculate_FDR($mainhash_all,0);
	...
	while($weight<2)
	{
		...
		($sum_target,$cutoff_score) = $self->calculate_FDR($mainhash_all,0.01);
...
sub calculate_FDR
{
	...
	foreach (reverse sort {$a<=>$b} keys %merge_score)
	{
			...
			my $FDR = ($sum_decoy*2)/($sum_target+$sum_decoy);
			if($FDR>$FDR_cutoff)
			{
				$cutoff_score = $_;
				last;
	...
	return ($sum_target,$cutoff_score);
}
