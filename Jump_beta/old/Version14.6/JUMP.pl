...
#	my $num_mass_region = 20 * $num_dynamic_mod;
	...
	$num_mass_region = $num_mass_region;
...
####################################################	
	...
	system(qq(rm -rf $database_path/temp_*mass*));	
	system(qq(rm -rf $database_path/temp_*pep*));
	system(qq(rm -rf $database_path/temp_*pro*));
		   ...
		   for(my $k=0;$k<=$#{$array->[$i]->[$j]};$k++)
			{
				for(my $l=0;$l<=$i;$l++)
				{
					for(my $m=0;$m<=$j;$m++)
					{
					   for(my $n=0;$n<=$k;$n++)
					   {
							$cum_array->[$i]->[$j]->[$k] += $array->[$l]->[$m]->[$n];
						}
					}
				}
				print OUTPUT $i,"\t",$j,"\t",$k,"\t",$cum_array->[$i]->[$j]->[$k],"\t",$cum_array->[$i]->[$j]->[$k]/$cum_array->[0]->[0]->[0],"\n";
			}
		}
	}
...
sub Create_Sort_BashFile
{
	...
	my $cmd = join(" ","perl $curr_dir/$dta_path/Create_Partial_Idx.pl -m",$Params->{"range"},"-job_num",$Params->{"JobNum"},"-dta_path",$Params->{"dta_path"},"-database_path",$Params->{"database_path"},"-mass_index",$Params->{"mass_index"},"-peptide_index", $Params->{"peptide_index"},"-protein_index",$Params->{"protein_index"},"-databasename",$Params->{"databasename"},"-num_pro_per_job",$Params->{"num_pro_per_job"},"-prot_job_num",$Params->{"prot_job_num"},"-inclusion_decoy",$Params->{"inclusion_decoy"},"-digestion",$Params->{"digestion"},"\n");
...
sub LuchParallelJob{
	...
	if($GridType eq 'SGE')
	{
		...
		print JOB $cmd;
		close(JOB);
		system(qq(qsub -cwd -pe mpi 64 -l mem_free=$mem_free,h_vmem=$max_mem $FileName >/dev/null 2>&1));
	}
	if($GridType eq 'PBS')
	{
