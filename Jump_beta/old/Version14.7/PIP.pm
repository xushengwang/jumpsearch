...
#		print $dtafile,"\n";
		...
		foreach my $order (keys %{$PIP->{$specscan}})
		{
			...
			my $prec_mz_array =  $PIP->{$specscan}->{$order}->{'mono_mz_array'};
						...
						{					
							for(my $k=2;$k<=3;$k++)
							{
								while($charge<4)
								{
									...
									for(my $i=1;$i<=$#inarray;$i++)
									{
										my @data = split(/\s+/,$inarray[$i]);
										if($temp_MH<$data[0])
										{
											$larger++;
										}
...
#####################################################			
			elsif(($PIP->{$specscan}->{$order}->{'pip'}*100)>$parameter->{'percentage_ppi'} and $parameter->{'max_num_ppi'} == 0)
			{
