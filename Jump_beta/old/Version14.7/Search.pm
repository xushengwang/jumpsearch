...
#			print $mod,"\t",$pepseq_core,"\t",$sideMassMatchResult->{'lSide_match'},"\t",$sideMassMatchResult->{'rSide_match'},"\t",$tags->{'tagSeq'},"gggggggggggggggggggggg\n";
			my ($tagSeqMatchResult,$sideMassMatchResult) = $self->matchTagSeq($pepseq,$pepseq_core,$mod,$tags,$frag_tolerance);
#			print $mod,"\t",$pepseq_core,"\t",$sideMassMatchResult->{'lSide_match'},"\t",$sideMassMatchResult->{'rSide_match'},"\t",$tags->{'tagSeq'},"fffffffffffffffffffff\n";
...
#	print $tags->{'tagSeq'},"kkkkkkkkkkkkkkkkkkkkkkkkkkkk\n";
	my $pseq = $pepseq_mod;
#	my $pseq = $pepseq_mod;
	$pseq =~ s/I/J/g;
	$pseq =~ s/L/J/g;
...
#	$pepseq =~ s/L/J/g;
	...
	#	$whole = index($pseq,$tseq,$whole+1);
		$whole = index($pseq,$tseq,$whole+1);
#		print $whole,"\t",$pseq,"\t",$tseq,"aaaaaaaaaaaa\n";
		if($whole < 0)
		{
		...
		{
#			print $sideMassMatch->{'lSide_match'},"\t",$sideMassMatch->{'rSide_match'},"\t",$whole,$pseq,"\t",$tseq,"bbbbbbbbbbbbbbbbbbbbbbb\n";
...
### commented by xusheng; only error without multiple with $tag->{'sideMass'}
#	print $pseq,"\t",$tag->{'tagSeq'},"\t",$lSide,"\t",$rSide,"\t",$tag->{'sideMass'},"\t",$result->{'lthreot_Mass'},"\t",$result->{'rthreot_Mass'},"uuuuuuuu\n";	
...
#### 6/28/2013 Fixed a bug: count multiple times because of similar theoretical mass generated from 1 charge and 2 charge	
	...
	foreach my $exp_mz_value (@$exp_mz)
	{
		if($tolerance_unit == 2)
		{
			$tolerance = $tolerance_input * $exp_mz_value / 1000000;
		}	
