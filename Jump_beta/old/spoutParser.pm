#!/usr/bin/perl -wT -I /home/yli4/lib/perl5
package spoutParser;
use strict;
use vars qw($Hydrogen_mass $HydrogenMinus_mass $Cterm_mass);# $now_date $now_time);
#use Spreadsheet::WriteExcel; # for .xls
use Excel::Writer::XLSX; # for xlsx

BEGIN
{
	($Hydrogen_mass, $HydrogenMinus_mass, $Cterm_mass)=(1.007825032,1.00335,19.017806);
	#($now_date, $now_time) = &gettime();
}
...
sub parseSpout
{
        ...
        for my $outfile (@out)
        {
                ...
                readOutfile($seqParaHash, $outInforHash, $run, $outfile);
        }
...
sub readOutfile
{
        my ($paraHash,$outInfor, $run, $outfile)=@_;
...
sub printPepXML
{
	...
	my ($now_date, $now_time) = ($self->{'now_date'},$self->{'now_time'});
        ...
        print XML '<msms_pipeline_analysis date="',"$now_date $now_time",'" ','summary_xml="',$outXML,'.pepXML">',"\n";
        ...
        for my $outfile (@out)
        {
                $count++;
                $outfile =~ s/(.*?)(\.spout)/$1$2/;
                unless (defined($$outInfor{$outfile}{'PPIpct'})) {die "XML:PPIpct not found in $outfile\n";}
                unless (defined($$outInfor{$outfile}{'prOrder'})) {die "XML:prOrder not found in $outfile\n";}
...
sub printXLSX
{
        my ($self, $seqPara, $outInfor, $run, $topMS2_array)=@_;
	my ($now_date, $now_time) = ($self->{'now_date'},$self->{'now_time'});
        ...
        my $worksheet0 = $workbook->add_worksheet('SummaryTable');
        $worksheet0->set_column('A:A',35);

        # Add a worksheet
        my $worksheet1 = $workbook->add_worksheet('PSM');
        #$worksheet1->set_column('D:E', 35);
        ...
        # 'Summary' worksheet
	if ($self->{'MS1scanNumber'} and $self->{'MS2scanNumber'}) {$self->{'MS1MS2ratio'}=$self->{'MS2scanNumber'}/$self->{'MS1scanNumber'};}
	...
	{
		$worksheet0->write($i0,0,"TOP$tmp",$titleFormat); $worksheet0->write($i0,1, $$topMS2_array[$tmp]); $i0++;
	}
	...
	_calculate_qValue(\%smallhash,'WeightedEvalue','total');

        #@out=sort(@out);
        my $i=0;
        $worksheet1->write($i,0,"Summary of Peptide Spectra Matches \(PSMs\)",$titleFormat); $i++;
        $i++;
        $worksheet1->write($i,0,"Program: Jump",$titleFormat); $i++;
        $worksheet1->write($i,0,"Date and time: $now_date $now_time",$titleFormat); $i++;
        $worksheet1->write($i,0,"Data set: $run",$titleFormat); $i++;
        $i++;

        my $j=0;
        $worksheet1->write($i,$j,"Order",$titleFormat); $j++;
        $worksheet1->write($i,$j,"Scan \#",$titleFormat); $j++;
        $worksheet1->write($i,$j,"PPI \#",$titleFormat); $j++;
        $worksheet1->write($i,$j,"Charge",$titleFormat); $j++;
         $worksheet1->write($i,$j,"Peptide Sequence",$titleFormat); $j++;
         $worksheet1->write($i,$j,"Protein",$titleFormat); $j++;
         $worksheet1->write($i,$j,"Weighted E value",$titleFormat); $j++;
        ...
        $worksheet1->write($i,$j,"Tag Sequence",$titleFormat); $j++;
         ...
         $worksheet1->write($i,$j,"Q value",$titleFormat); $j++;
	...
	my (@precursorMass,$pcmI);$pcmI=0;
        ...
        for my $outfile (sort {$smallhash{$b}{WeightedEvalue} <=> $smallhash{$a}{WeightedEvalue}} keys %smallhash)
        {
                ...
                $i++;$count++;$j=0;
                $worksheet1->write($i,$j,$count,$ctrFormat);  $j++;
                $worksheet1->write($i,$j,$$outInfor{$outfile}{'scan'}); $j++;
                $worksheet1->write($i,$j,$$outInfor{$outfile}{'PPI'},$ctrFormat); $j++;
                $worksheet1->write($i,$j,$$outInfor{$outfile}{'charge'},$ctrFormat); $j++;
                $worksheet1->write($i,$j,"$$outInfor{$outfile}{'prAA'}[1]\.$$outInfor{$outfile}{'fullPeptide'}[1]\.$$outInfor{$outfile}{'nxAA'}[1]"); $j++;
                $worksheet1->write($i,$j,$$outInfor{$outfile}{'protein'}[1]); $j++;
                $worksheet1->write($i,$j,$$outInfor{$outfile}{'WeightedEvalue'}[1]); $j++;
	...
	$pcmSlot=10;
	for (my $k=0; $k<=int($precursorMass[scalar(@precursorMass)-1]/$pcmSlot); $k++) { $pcmArray[$k]=0; }
...
=cut
        ...
        my $markBin=0;
        for (my $k=0; $k<$binN; $k++) { if  ($wev{'targetCml'}[$k]/$wev{'targetCml'}[$binN-1]>=$cutPct) {$markBin=$k; last;} }
        ...
        my $wevChart = $workbook->add_chart( type => 'line', name => 'Weighted evalue distribution', embedded => 1);$wevChart->set_title( name => 'Weighted E value distribution' );

        #$i=$i+20;
        $i=$markRows[5]-1;
        $worksheet2->write($i,$colOffset,'Weighted E value',$titleFormat);$i++;
	...
	$a1='B'; $a2='C';
        ...
        {
                $wev{'fdr'}[$k]=$wev{'decoyRevCml'}[$k]*2/($wev{'targetRevCml'}[$k]+$wev{'decoyRevCml'}[$k]);
        }
=head
        for (my $k=0; $k<$binN; $k++)
        {
